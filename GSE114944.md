...
file.choose()
expression_m <- read.table("TIL gene symbol.txt", header = T, sep = '\t', row.names = 1)
...
```r
expression_m <- read.table("TIL gene symbol.txt", header = T, sep = '\t', row.names = 1)
expression_m <- as.matrix(expression_m)

# apply KNN
imputed_expression <- impute.knn(expression_m, k = 10, rowmax = 0.5, colmax = 0.8, maxp = 3000, rng.seed = 362436069)
...
# output results
expression_final <- imputed_expression
genesymbol <- rownames(expression_final)
expression_final <- cbind(genesymbol, expression_final)
write.table(expression_final, file = "TIL expression final.txt", sep = '\t', quote = F, row.names = F)
```
