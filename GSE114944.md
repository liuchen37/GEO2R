...
By doing this step, the following table consists of three groups of gene expression data was created:

![alt text](https://github.com/liuchen37/Pics/blob/main/expression114944.jpg?raw=true)

## Differential expression

Remove excess columns in the chip label file before processing, keep only ID, gene symbol and gene title.

![alt text](https://github.com/liuchen37/Pics/blob/main/GPL%20processing.jpg?raw=truehttps://github.com/liuchen37/Pics/blob/main/GPL%20processing.jpg?raw=true)
...
file.choose()
setwd("/Users/chenliu/Documents/GEO/GSE114944/")

# load expression and GPL file
probe_exp <- read.table("expression.txt", header = T, sep = "\t", row.names = 1)
probe_info <- read.table("GPL15207-17536.txt", header = T, sep = "\t")
probe_name <- rownames(probe_exp)

# match probeid with gene name, filter
